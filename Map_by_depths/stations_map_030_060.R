...
stations_data <- lookup_table %>%
  filter(!is.na(Latitude) & !is.na(Longitude) & Avg_Depth >= 0.30 & Avg_Depth <= 0.60) %>%
...
map_plot <- ggplot() +
  ...
  labs(x = "Longitude", y = "Latitude", 
       title = "Spatial Distribution of Stations with Depth of 30 - 60cm") +
  theme_minimal()
...
ggsave("/home/xosto002/Beachelor_thesis/spatial_map_depth/stations_map_0.30-0.60.pdf", plot = map_plot, width = 10, height = 6)
