...
stations_data <- lookup_table %>%
  filter(!is.na(Latitude) & !is.na(Longitude) & Number_of_Full_Years >= 5) %>%
...
map_plot <- ggplot() +
  ...
  labs(x = "Longitude", y = "Latitude", 
       title = "Spatial Distribution of Stations with Data of at least 5 Year") +
  theme_minimal()
...
ggsave("/home/xosto002/Beachelor_thesis/spatial_map/stations_map_5year.pdf", plot = map_plot, width = 10, height = 6)
