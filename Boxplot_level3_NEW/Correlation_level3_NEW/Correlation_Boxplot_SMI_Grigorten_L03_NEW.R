...
output_dir <- "C:/Users/Ognjen/Desktop/Beachelor thesis/Beachelor_thesis/Beachelor_thesis/Project/BoxPlots/Grigorten_Correlation_L03"

if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

# Read lookup table and filter by Avg_Depth for Level 3
...
lookup_table <- lookup_table %>% filter(Avg_Depth >= 0.15 & Avg_Depth <= 0.3)
...
res_folders <- list(
  ...
  "0p03125deg"  = "mhm_0p03125deg_L01_L02_L03",
...
calculate_correlation <- function(obs, sim) {
  ...
  cor(obs_filtered$SMI, sim_filtered$SMI, use = "complete.obs")
}
...
for (station_id in unique(lookup_table$ID)) {
  ...
  for (res_code in names(res_folders)) {
    if (res_code == "0p015625deg") {
      ...
      filename_pattern <- sprintf("SMvol_L03_%s_ID_%d_new.nc", res_code, station_id)
    } else {
      folder_path <- file.path(sim_base_dir, res_folders[[res_code]])
      filename_pattern <- sprintf("SMvol_L03_%s_ID_%d.nc", res_code, station_id)
    }
    ...
    if (!is.null(sim_data)) {
      corr_value <- calculate_correlation(obs_data, sim_data)
...
write.csv(correlation_results, sprintf("%s/Correlation_results_SMI_L03.csv", output_dir), row.names = FALSE)
...
ggplot(correlation_results, aes(x = factor(Resolution, levels = resolution_order), y = Correlation, fill = Resolution)) +
  ...
  labs(title = "Correlation Boxplot for Different Resolutions (Level 3) - SMI", x = "Resolution", y = "Correlation (SMI)") +
  theme_minimal() +
  theme(legend.position = "none")

ggsave(sprintf("%s/Correlation_Boxplot_SMI_Level3.pdf", output_dir), width = 10, height = 6)
cat("Boxplot saved:", sprintf("%s/Correlation_Boxplot_SMI_Level3.pdf", output_dir), "\n")

# Count used stations
num_used_stations <- length(unique(correlation_results$Station_ID))
cat(" Number of stations used in correlation calculation:", num_used_stations, "\n")
