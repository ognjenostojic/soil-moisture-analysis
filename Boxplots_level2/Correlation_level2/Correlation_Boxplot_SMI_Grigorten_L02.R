...
output_dir <- "/home/xosto002/Beachelor_thesis/Project/BoxPlots_Level2/Grigorten_Correlation_L02"
...
simulation_dir_3km <- "/DATA/theses/ognjen/processed/mhm_timeseries_1998_2022_NEW_NOT_USE_TMP_FOLDER"

# Define Level 2 resolutions
resolutions <- c("0p015625deg" = "1km", 
                 "0p03125deg"  = "3km",  # Taken from NEW folder
                 "0p0625deg"   = "5km", 
                 "0p125deg"    = "11km", 
                 "0p25deg"     = "22km")
...
for (station_id in unique(lookup_table$ID)) {
  ...
  for (res in names(resolutions)) {
    sim_file_path <- ifelse(res == "0p03125deg",
                            sprintf("%s/SMvol_L02_%s_ID_%d_new.nc", simulation_dir_3km, res, station_id),  # NEW folder for 3km
                            sprintf("%s/SMvol_L02_%s_ID_%d.nc", simulation_dir_main, res, station_id))  # Main folder for others
