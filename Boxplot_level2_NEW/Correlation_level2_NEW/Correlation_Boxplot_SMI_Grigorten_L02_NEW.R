...
output_dir <- "C:/Users/Ognjen/Desktop/Beachelor thesis/Beachelor_thesis/Beachelor_thesis/Project/BoxPlots/Grigorten_Correlation_L02"

if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

# Read and filter lookup table for Level 2 depths (0.05–0.15)
...
for (station_id in unique(lookup_table$ID)) {
  ...
  for (res_code in names(res_folders)) {
    # File path logic for 3km special case
...
write.csv(correlation_results, sprintf("%s/Correlation_results_SMI_L02.csv", output_dir), row.names = FALSE)
...
ggplot(correlation_results, aes(x = factor(Resolution, levels = resolution_order), y = Correlation, fill = Resolution)) +
  ...
  labs(title = "Correlation Boxplot for Different Resolutions (Level 2) - SMI", x = "Resolution", y = "Correlation (SMI)") +
  theme_minimal() +
