...
output_dir <- "C:/Users/Ognjen/Desktop/Beachelor thesis/Beachelor_thesis/Beachelor_thesis/Project/BoxPlots/Grigorten_RMSE_L01_NEW"
...
ggplot(rmse_results, aes(x = factor(Resolution, levels = resolution_order), y = RMSE, fill = Resolution)) +
  ...
  labs(title = "RMSE Boxplot for Different Resolutions (Level 1) - SMI", x = "Resolution", y = "RMSE (SMI)") +
  theme_minimal() +
  theme(legend.position = "none")

ggsave(sprintf("%s/RMSE_Boxplot_SMI_Level1.pdf", output_dir), width = 10, height = 6)
cat("Boxplot saved:", sprintf("%s/RMSE_Boxplot_SMI_Level1.pdf", output_dir), "\n")


# Count number of unique stations used in RMSE calculation
