...
output_dir <- "C:/Users/Ognjen/Desktop/Beachelor thesis/Beachelor_thesis/Beachelor_thesis/Project/BoxPlots/Grigorten_Correlation_L01_NEW"

if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

# Read and filter lookup table by Avg_Depth
...
lookup_table <- lookup_table %>% filter(Avg_Depth >= 0 & Avg_Depth <= 0.05)

# Define parent directory for resolution folders
sim_base_dir <- "C:/Users/Ognjen/Desktop/theses/ognjen/processed"

# Map folder names to resolution codes
res_folders <- list(
...
for (station_id in unique(lookup_table$ID)) {
  ...
  for (res_code in names(res_folders)) {
    if (res_code == "0p015625deg") {
      ...
      filename_pattern <- sprintf("SMvol_L01_%s_ID_%d_new.nc", res_code, station_id)
    } else {
      folder_path <- file.path(sim_base_dir, res_folders[[res_code]])
      filename_pattern <- sprintf("SMvol_L01_%s_ID_%d.nc", res_code, station_id)
    }
