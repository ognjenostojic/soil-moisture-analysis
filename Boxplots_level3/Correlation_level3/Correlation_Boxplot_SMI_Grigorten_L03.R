...
output_dir <- "/home/xosto002/Beachelor_thesis/Project/BoxPlots_Level3/Grigorten_Correlation_L03"
...
for (station_id in unique(lookup_table$ID)) {
  ...
  for (res in names(resolutions)) {
    sim_file_path <- ifelse(res == "0p015625deg",
                            sprintf("%s/SMvol_L03_%s_ID_%d_new.nc", simulation_dir_1km, res, station_id),  # NEW folder for 1km
                            sprintf("%s/SMvol_L03_%s_ID_%d.nc", simulation_dir_main, res, station_id))  # Main folder for others
