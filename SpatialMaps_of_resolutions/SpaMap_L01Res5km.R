...
# Find the NetCDF file for 5km resolution (0p0625deg)
files <- list.files(pattern = "SMvol_L01_0p0625deg\\.nc", full.names = TRUE)
...
p <- ggplot(data = df, aes(x = lon, y = lat, fill = soil_moisture)) +
  ...
  labs(fill = "Soil Moisture", x = "Longitude", y = "Latitude",
       title = "Soil Moisture Distribution - 5km (Level 1)") +
  theme_minimal()

# Define output file path
output_file <- sprintf("%s/Soil_Moisture_Map_L1_5km.pdf", output_dir)
