...
# Find the NetCDF file for 11km resolution (0p125deg)
files <- list.files(pattern = "SMvol_L01_0p125deg\\.nc", full.names = TRUE)

# Check if the file exists
if (length(files) == 0) {
  stop("⚠️ No file found for 11km resolution.")
}
...
p <- ggplot(data = df, aes(x = lon, y = lat, fill = soil_moisture)) +
  ...
  labs(fill = "Soil Moisture", x = "Longitude", y = "Latitude",
       title = "Soil Moisture Distribution - 11km (Level 1)") +
  theme_minimal()

# Define output file path
output_file <- sprintf("%s/Soil_Moisture_Map_L1_11km.pdf", output_dir)
