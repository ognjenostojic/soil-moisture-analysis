...
# Find the NetCDF file for 22km resolution (0p25deg)
files <- list.files(pattern = "SMvol_L01_0p25deg\\.nc", full.names = TRUE)

# Check if the file exists
if (length(files) == 0) {
  stop("⚠️ No file found for 22km resolution.")
}
...
p <- ggplot(data = df, aes(x = lon, y = lat, fill = soil_moisture)) +
  ...
  labs(fill = "Soil Moisture", x = "Longitude", y = "Latitude",
       title = "Soil Moisture Distribution - 22km (Level 1)") +
  theme_minimal()

# Define output file path
output_file <- sprintf("%s/Soil_Moisture_Map_L1_22km.pdf", output_dir)
