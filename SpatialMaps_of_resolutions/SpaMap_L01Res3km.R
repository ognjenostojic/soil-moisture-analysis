...
# Find the NetCDF file for 3km resolution (0p03125deg)
files <- list.files(pattern = "SMvol_L01_0p03125deg\\.nc", full.names = TRUE)

# Check if the file exists
if (length(files) == 0) {
  stop("⚠️ No file found for 3km resolution.")
}
...
p <- ggplot(data = df, aes(x = lon, y = lat, fill = soil_moisture)) +
  ...
  labs(fill = "Soil Moisture", x = "Longitude", y = "Latitude",
       title = "Soil Moisture Distribution - 3km (Level 1)") +
  theme_minimal()

# Define output file path
output_file <- sprintf("%s/Soil_Moisture_Map_L1_3km.pdf", output_dir)
